(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();var e=class{constructor(e){this.parent=e}getHTML(){return`
        ...
        `}addOrder(){if(this.orders.length===0)return;let e=this.orders[0],t=Math.max(...this.orders.map(e=>e.id))+1,n=new Date().toISOString().slice(0,10),r={id:t,name:`${e.name} (копия)`,date:n};this.orders.push(r),this.renderOrders()}deleteOrder(e){this.orders=this.orders.filter(t=>t.id!==e),this.renderOrders()}renderOrders(){let e=document.getElementById(`orders-list`);e&&(e.innerHTML=this.orders.map(e=>`
            <div class="order-card">
                <div class="order-number">Заказ #${e.id}</div>
                <div class="order-name">${e.name}</div>
                <div class="order-date">Дата заказа: ${e.date}</div>
                <button class="btn-delete-order" data-id="${e.id}">🗑 Удалить</button>
            </div>
        `).join(``),document.querySelectorAll(`.btn-delete-order`).forEach(e=>{e.addEventListener(`click`,()=>{let t=parseInt(e.getAttribute(`data-id`));this.deleteOrder(t)})}))}goToHome(){new u(this.parent).render()}render(){this.parent.innerHTML=``,new e(this.parent).render(this.goToHome.bind(this),()=>{});let t=this.getHTML();this.parent.insertAdjacentHTML(`beforeend`,t);let n=document.getElementById(`add-order-button`);n&&n.addEventListener(`click`,()=>this.addOrder()),this.renderOrders()}},a=new class{async get(e){let t=await fetch(e);if(!t.ok)throw Error(`HTTP ${t.status}`);return t.json()}async post(e,t){let n=await fetch(e,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify(t)});if(!n.ok)throw Error(`HTTP ${n.status}`);return n.json()}async patch(e,t){let n=await fetch(e,{method:`PATCH`,headers:{"Content-Type":`application/json`},body:JSON.stringify(t)});if(!n.ok)throw Error(`HTTP ${n.status}`);return n.json()}async delete(e){let t=await fetch(e,{method:`DELETE`});if(!t.ok)throw Error(`HTTP ${t.status}`);if(t.status!==204)return t.json()}},o=new class{constructor(){this.baseUrl=`http://localhost:3000`}getStocks(e=``){let t=`${this.baseUrl}/datacenter`;return e&&(t+=`?search=${encodeURIComponent(e)}`),t}getStockById(e){return`${this.baseUrl}/datacenter/${e}`}createStock(){return`${this.baseUrl}/datacenter`}removeStockById(e){return`${this.baseUrl}/datacenter/${e}`}updateStockById(e){return`${this.baseUrl}/datacenter/${e}`}addComment(e){return`${this.baseUrl}/datacenter/${e}/comments`}};function s(e){return e?e.replace(/&/g,`&amp;`).replace(/</g,`&lt;`).replace(/>/g,`&gt;`).replace(/"/g,`&quot;`).replace(/'/g,`&#39;`):``}var c=class{constructor(e,t){this.parent=e,this.id=parseInt(t),this.pollingInterval=null}get pageRoot(){return document.getElementById(`product-page`)}getHTML(){return`
